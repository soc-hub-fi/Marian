//------------------------------------------------------------------------------
// Module   : canright_sbox_tb.sv
//
// Project  : Vector-Crypto Subsystem (Marian)
// Author(s): Tom Szymkowiak <thomas.szymkowiak@tuni.fi>
// Created  : 07-dec-2023
//
// Description: Testbench for canright_sbox module
//
// Parameters:
//  - None
//
 ...
 ********************************/
 ...
 // table-based forward/inverse sbox implementations for reference
 ...
 *************/
  ...
  // DUT interface
  logic [7:0] dut_input_byte;
  logic [7:0] dut_output_byte;
  logic       dut_encrypt_sel;
 ...
 *********/
  ...
  canright_sbox i_dut (
    .sbox_byte_i   ( dut_input_byte  ),
    .encrypt_sel_i ( dut_encrypt_sel ),
    .sbox_byte_o   ( dut_output_byte )
  );
 ...
 *******************************/
  ...
  initial begin : tb_logic
    ...
    repeat(255) begin
      ...
      if (dut_output_byte != SboxFwd[dut_input_byte]) begin
        $error("Forward sbox output is incorrect!\nExpected: 0x%H\nResult: 0x%H",
          SboxFwd[dut_input_byte], dut_output_byte);
        tb_err_count++;
      end

      dut_encrypt_sel = 1'b0;

      @(posedge tb_clk);

      if (dut_output_byte != SboxInv[dut_input_byte]) begin
        $error("Inverse sbox output is incorrect!\nExpected: 0x%H\nResult: 0x%H",
          SboxInv[dut_input_byte], dut_output_byte);
        tb_err_count++;
        end

      dut_input_byte = dut_input_byte + 1;

    end

    // end simulation
    $display("%0tns - Simulation Complete!", $time);
    if (tb_err_count != 0) begin
      $info("Test ", $sformatf("*** [FAILED] *** (tohost = %0d)", 1));
    end else begin
      $info("Test ", $sformatf("*** [PASSED] *** (tohost = %0d)", 0));
    end
    $finish;
