...
// Created  : 08-jun-2024
//
// Description: SIMD ALU modified for use within Marian. Original
  ...
  always_comb begin: p_alu
    ...
    if (valid_i)
      unique case (op_i)
        ...
        VANDN:         res = ~(operand_a_i) & operand_b_i;
          ...
          case (vew_i)
            EW8 : begin
              for (int b = 0; b < 8; b++) begin
                  res.w8[b] = opb.w8[b]; // return operand as element size is 1 byte
              end
            end
            EW16: begin
              for (int b = 0; b < 4; b++) begin
                for (int b_i = 0; b_i < 2; b_i++) begin
                  res.w16[b][(8*b_i) +: 8] = opb.w16[b][(16-((b_i+1)*8)) +: 8];
                end
              end
            end
            EW32: begin
              for (int b = 0; b < 2; b++) begin
                for (int b_i = 0; b_i < 4; b_i++) begin
                  res.w32[b][(8*b_i) +: 8] = opb.w32[b][(32-((b_i+1)*8)) +: 8];
                end
              end
            end
            EW64: begin
              for (int b = 0; b < 1; b++) begin
                for (int b_i = 0; b_i < 8; b_i++) begin
                  res.w64[b][(8*b_i) +: 8] = opb.w64[b][(64-((b_i+1)*8)) +: 8];
                end
              end
            end
            default:
        ...
        VROL: begin
          case (vew_i)
            EW8 : begin
              for (int b = 0; b < 8; b++) begin
                automatic logic[7:0] rot_amount = opa.w8[b] & (8'd7);
                automatic logic[7:0] rot_upper = opb.w8[b] << int'(rot_amount);
                automatic logic[7:0] rot_lower = opb.w8[b] >> (8 - int'(rot_amount));
                res.w8[b] = rot_upper | rot_lower;
              end
            end
            EW16: begin
              for (int b = 0; b < 4; b++) begin
                automatic logic[15:0] rot_amount = opa.w16[b] & (16'd15);
                automatic logic[15:0] rot_upper = opb.w16[b] << int'(rot_amount);
                automatic logic[15:0] rot_lower = opb.w16[b] >> (16 - int'(rot_amount));
                res.w16[b] = rot_upper | rot_lower;
              end
            end
            EW32: begin
              for (int b = 0; b < 2; b++) begin
                automatic logic[31:0] rot_amount = opa.w32[b] & (32'd31);
                automatic logic[31:0] rot_upper = opb.w32[b] << int'(rot_amount);
                automatic logic[31:0] rot_lower = opb.w32[b] >> (32 - int'(rot_amount));
                res.w32[b] = rot_upper | rot_lower;
              end
            end
            EW64: begin
              for (int b = 0; b < 1; b++) begin
                automatic logic[63:0] rot_amount = opa.w64[b] & (64'd63);
                automatic logic[63:0] rot_upper = opb.w64[b] << int'(rot_amount);
                automatic logic[63:0] rot_lower = opb.w64[b] >> (64 - int'(rot_amount));
                res.w64[b] = rot_upper | rot_lower;
              end
            end
            default:
              res = '0;
        ...
        VROR: begin
          case (vew_i)
            EW8 : begin
              for (int b = 0; b < 8; b++) begin
                automatic logic[7:0] rot_amount = opa.w8[b] & (8'd7);
                automatic logic[7:0] rot_upper = opb.w8[b] << (8 - int'(rot_amount));
                automatic logic[7:0] rot_lower = opb.w8[b] >> int'(rot_amount);
                res.w8[b] = rot_upper | rot_lower;
              end
            end
            EW16: begin
              for (int b = 0; b < 4; b++) begin
                automatic logic[15:0] rot_amount = opa.w16[b] & (16'd15);
                automatic logic[15:0] rot_upper = opb.w16[b] << (16 - int'(rot_amount));
                automatic logic[15:0] rot_lower = opb.w16[b] >> int'(rot_amount);
                res.w16[b] = rot_upper | rot_lower;
              end
            end
            EW32: begin
              for (int b = 0; b < 2; b++) begin
                automatic logic[31:0] rot_amount = opa.w32[b] & (32'd31);
                automatic logic[31:0] rot_upper = opb.w32[b] << (32 - int'(rot_amount));
                automatic logic[31:0] rot_lower = opb.w32[b] >> int'(rot_amount);
                res.w32[b] = rot_upper | rot_lower;
              end
            end
            EW64: begin
              for (int b = 0; b < 1; b++) begin
                automatic logic[63:0] rot_amount = opa.w64[b] & (64'd63);
                automatic logic[63:0] rot_upper = opb.w64[b] << (64 - int'(rot_amount));
                automatic logic[63:0] rot_lower = opb.w64[b] >> int'(rot_amount);
                res.w64[b] = rot_upper | rot_lower;
              end
            end
            default:
